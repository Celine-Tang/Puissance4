...
tableau = grille_vide()
joueur = int(input("Tu es le joueur 1 ou 2? "))
colonne = int(input("Dans quel colonne veux-tu jouer? "))
if coup_possible(tableau,colonne)==True:
    ligne=jouer(tableau,joueur,colonne)
else:
    print("Coup impossible")
affiche(tableau)
while victoire(tableau,joueur,ligne,colonne) != True and match_nul(tableau) != True:
    joueur = int(input("Tu es le joueur 1 ou 2? "))
    colonne = int(input("Dans quel colonne veux-tu jouer? "))
    if coup_possible(tableau,colonne) == True:
        ligne = jouer(tableau,joueur,colonne)
